...
class NewsDataIO
{
    ...
    public function __construct(Settings $settings)
    {
        ...
        add_action('in_admin_header', [$this, 'adminHeader'], 10, 0);
        add_action('wp_enqueue_scripts', [$this, 'enqueueAssets'], 10, 0);
        add_action('admin_post_newsdata_io_save_settings', fn () => $this->saveSettings($_POST), 10, 0);
    ...
    public function saveSettings(array $args): void
    {
        ...
        if (!current_user_can('manage_options')) {
            wp_die(esc_attr__('You are not allowed to perform this action.', 'newsdata-io'));
        }
    ...
    {
        if (get_current_screen() === null || get_current_screen()->id !== 'toplevel_page_newsdata-io') {
            return;
        }
        ?>
        <style type="text/css">
    ...
    {
        $currentTab = filter_input(INPUT_GET, 'tab', FILTER_UNSAFE_RAW, FILTER_NULL_ON_FAILURE);
        if ($currentTab === 'latest' || !$currentTab) {
            require_once __DIR__ . '/templates/settings.php';
        } else {
            require_once __DIR__ . '/templates/' . $currentTab . '.php';
        }
    }
