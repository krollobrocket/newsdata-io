...
namespace Cyclonecode\NewsDataIO\Plugin\Settings;
...
class Settings implements \IteratorAggregate, \Countable, \ArrayAccess
{
    ...
    {
        return function_exists('json_encode') ? wp_json_encode($this->toOptionsArray(), JSON_PRETTY_PRINT) : '';
    }
    ...
    {
        $this->settings = (array) get_option($this->optionName, []);
     ...
     * @return bool
     */
    public function saveToFile(string $filename, string $format = 'json'): bool
    {
        $content = '';
        switch ($format) {
            case 'raw':
        ...
        $this->fileSystem->put_contents($filename, $content ?: '');
